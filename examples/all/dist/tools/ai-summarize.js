...
// tools/ai-summarize.ts
var ai_summarize_exports = {};
__export(ai_summarize_exports, {
  POST: () => POST,
  metadata: () => metadata,
  schema: () => schema
});
module.exports = __toCommonJS(ai_summarize_exports);
...
function buildMetadataPayload(base, toolExecution, extras) {
  ...
  if (toolExecution) {
    ...
    metadata2.openpond = openpond;
  }
  return Object.keys(metadata2).length > 0 ? metadata2 : void 0;
}
...
var schema = z.object({
  ...
  focus: z.string().optional().describe("Key angle or audience focus for the summary"),
  tone: z.enum(["neutral", "enthusiastic", "technical", "casual"]).default("neutral").describe("Desired tone of the summary"),
  maxTokens: z.number().int().min(200).max(1200).default(400).describe("Token limit for the generated summary"),
  model: z.string().optional().describe("Override default model (e.g., 'claude-4-sonnet-20250514')")
});
var metadata = {
  name: "ai_summarize",
  description: "Generate concise summaries tailored to different tones and audiences",
  annotations: {
    readOnlyHint: true,
    ...
    examples: [
      {
        ...
        expectedOutput: "A short paragraph highlighting what the AI package enables and why it matters."
      },
      {
        ...
        expectedOutput: "Detailed explanation that references stream mechanics and abort behavior."
      }
    ],
  ...
  const response = await generateText(
    {
      messages: [
        {
          role: "system",
          content: "You are a helpful technical writer. Summaries should be factual, concise, and respect the requested tone."
        },
        {
          role: "user",
          content: [
            `Topic: ${payload.topic}`,
            `Tone: ${payload.tone}`,
            payload.focus ? `Focus: ${payload.focus}` : void 0,

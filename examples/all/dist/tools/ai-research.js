...
// tools/ai-research.ts
var ai_research_exports = {};
__export(ai_research_exports, {
  POST: () => POST,
  metadata: () => metadata,
  schema: () => schema
});
module.exports = __toCommonJS(ai_research_exports);
...
var schema = z.object({
  query: z.string().min(5).describe("Research question or topic to investigate"),
  maxResults: z.number().int().min(1).max(10).default(5).describe("Maximum number of web results to inspect"),
...
var metadata = {
  name: "ai_research",
  description: "Investigate a topic using the gateway websearch tool and return sourced findings",
  annotations: {
    readOnlyHint: true,
    ...
    examples: [
      {
        ...
        expectedOutput: "Short narrative with bullet citations pointing to authoritative sources."
      },
      {
        ...
        expectedOutput: "Structured analysis referencing each source and highlighting strengths/risks."
      }
    ],
  ...
  const response = await generateText(
    {
      messages: [
        {
          role: "system",
          content: "You are an investigative assistant. Use the websearch tool when the user asks for current information."
        },
        {
          role: "user",
          content: [

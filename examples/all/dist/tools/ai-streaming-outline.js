...
// tools/ai-streaming-outline.ts
var ai_streaming_outline_exports = {};
__export(ai_streaming_outline_exports, {
  POST: () => POST,
  metadata: () => metadata,
  schema: () => schema
});
module.exports = __toCommonJS(ai_streaming_outline_exports);
...
var schema = z.object({
  ...
  bulletCount: z.number().int().min(3).max(12).default(6).describe("How many bullet points to include"),
  includeSummary: z.boolean().default(true).describe("Whether to append a short summary after the bullet list")
});
var metadata = {
  name: "ai_streaming_outline",
  description: "Demonstrate streaming outline generation with incremental deltas",
  annotations: {
    readOnlyHint: true,
    ...
    examples: [
      {
        ...
        expectedOutput: "Outline text streamed in chunks, with a concluding summary paragraph."
      }
    ],
...
async function POST(request) {
    ...
    const { finished } = await streamText(
      {
        messages: [
          {
            role: "system",
            content: "You produce well-structured outlines. Stream bullet items as they are ready, then append an optional summary."
          },
          {
            role: "user",
            content: [
              ...
              payload.includeSummary ? "Include a short summary paragraph." : void 0
            ].filter(Boolean).join("\n")
          }
        ],
        generation: {
  ...
  return Response.json({
    ...
    reasoning: reasoningChunks.length > 0 ? reasoningChunks.join("") : void 0,
    toolSignals: toolSignals.length > 0 ? toolSignals : void 0,
    usage
  });
}

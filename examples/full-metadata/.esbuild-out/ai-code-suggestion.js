...
// tools/ai-code-suggestion.ts
var aiConfig = {
  baseUrl: process.env.OPENPOND_GATEWAY_URL,
  apiKey: process.env.OPENPOND_API_KEY
};
var schema = external_exports.object({
  language: external_exports.enum(["typescript", "python", "rust", "go", "bash"]).describe("Target programming language"),
  task: external_exports.string().min(5).describe("What the code snippet should accomplish"),
  constraints: external_exports.array(external_exports.string().min(1)).default([]).describe("Additional requirements, such as libraries or style"),
  tests: external_exports.boolean().default(true).describe("Whether to include a lightweight test or usage example")
});
var metadata = {
  name: "ai_code_suggestion",
  description: "Draft language-specific code snippets with optional usage examples",
  annotations: {
    readOnlyHint: true,
    ...
    examples: [
      {
        ...
        expectedOutput: "Complete function with inline documentation and optional usage block."
      },
      {
        ...
        expectedOutput: "Self-contained CLI using argparse with error handling."
      }
    ],
  ...
  const response = await generateText(
    {
      messages: [
        {
          role: "system",
          content: "You are an expert software engineer. Provide well-commented code that follows modern best practices."
        },
        {
          role: "user",
          content: [

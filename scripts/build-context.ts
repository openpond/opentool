#!/usr/bin/env bun
/**
 * Build-time script to bundle opentool documentation and source code
...
function main() {
  ...
  const walletSource = {
    ...Object.fromEntries(
      Object.entries(loadDirectory(path.join(ROOT_DIR, "src/wallet"))).map(
        ([name, content]) => [`src/wallet/${name}`, content]
      )
    ),
    ...Object.fromEntries(
      Object.entries(loadDirectory(path.join(ROOT_DIR, "src/wallets"))).map(
        ([name, content]) => [`src/wallets/${name}`, content]
      )
    ),
  };
  let version = "0.0.0";
...
${Object.entries(walletSource)
  .map(
    ([name, content]) =>
      `  ${JSON.stringify(name)}: \`${escapeForTemplate(
        content
      )}\`,`
  )
  .join("\n")}
